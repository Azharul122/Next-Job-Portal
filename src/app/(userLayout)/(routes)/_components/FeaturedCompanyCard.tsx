...
const FeaturedCompanyCard = ({ company }: propsTypes) => {
    ...
    return (
        <div key={company.id} className={`flex items-center   gap-2  text-lg font-bold  px-3 py-2 dark:shadow-xl backdrop-blur-md cursor-pointer hover:rounded-full`} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} >
            <div className="size-28 flex justify-center items-center border-2 dark:border-slate-700 border-light-border dark:bg-slate-800/20 bg-lightbg rounded-full  ">
                <div className="size-24 rounded-full border-2 border-slate-700 relative flex justify-center items-center ">
                    ...
                    {
                        hover && (
                            <div onClick={() => { router.push(`/search?company=${company?.id}`) }} className="absolute inset-0 flex justify-center items-center bg-transparentBGDarkprimary dark:bg-transparentBGprimary z-30 rounded-full" >
