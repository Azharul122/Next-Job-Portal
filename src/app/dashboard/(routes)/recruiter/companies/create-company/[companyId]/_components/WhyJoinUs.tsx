...
const formSchema = z.object({
    whyJoinUs: z.string().min(100, {
        message: "whyJoinUs must be at least 100 characters.",
    }),
})
...
const WhyJoinUs = ({ companyId, initialCompany, isRequired }: propsTypes) => {
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            whyJoinUs: initialCompany.whyJoinUs || ""
        }
    })
    ...
    const handlePromptGenerate = async () => {
        ...
        try {
            setIsPrompting(true)
            const customPrompts = `could you suggest why Join Us to our   ${prompt ? prompt : initialCompany.companyTitle} company within 250 words including vision,reservasion with a point where each point will be bold font`
    ...
    return (
        <div className='border px-2 pb-4 mt-3 dark:bg-darkbg bg-lightbg'>
            <div className="flex justify-between items-center py-4 text-lg">
                <p>WhyJoinUs <sup className={`${isRequired ? "text-red-500" : "hidden"}`}>*</sup></p>
            ...
            </div>
            {
                editing && <div className="">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {
                            initialCompany.whyJoinUs && <Repeat2 onClick={() => handlePromptGenerate()} />
                        }
                    </div>
                    {
                        geminiData && (
            ...
            }
            {
                !editing && (<div className='text-sm'>
                    <Preview value={initialCompany.whyJoinUs} />
                </div>)
            }
        </div>
    )
