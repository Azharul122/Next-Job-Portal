...
const CompanyCoverImage = ({ companyId, initialCompany }: propsTypes) => {
    const [editing, setEditing] = useState(false)
    const form = useForm<z.infer<typeof formSchema>>({
        resolver: zodResolver(formSchema),
        defaultValues: {
            coverImage: initialCompany?.coverImage || ""
        }
    })
    ...
    return (
        <div className='border px-2 pb-4 mt-3 bg-lightbg dark:bg-darkbg'>
            ...
            {
                !editing && (
                    !initialCompany?.coverImage ? <div className="h-60 flex items-center justify-center">
                        <ImageIcon className='size-10' />
                    </div> : <div className="relative aspect-video mt-2">
                        <Image src={initialCompany?.coverImage} className='size-full' fill alt='Image' />
                    </div>
                )
            }
        </div>
