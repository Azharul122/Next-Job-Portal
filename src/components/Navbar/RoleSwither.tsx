...
const RoleSwither = () => {
    ...
    return (
        <Link href={isRouter ? "/dashboard/recruiter" : "/dashboard/admin"} className="relative px-5 py-1 font-medium text-white group">
            <span className={`absolute inset-0 w-full h-full transition-all duration-300 ease-out transform translate-x-0 -skew-x-12 ${isRouter ? "bg-gradient-start group-hover:bg-gradient-end" : "bg-gradient-end group-hover:bg-gradient-start"} group-hover:skew-x-12`}></span>
            <span className={`absolute inset-0 w-full h-full transition-all duration-300 ease-out transform skew-x-12 ${!isRouter ? "bg-gradient-start group-hover:bg-gradient-end" : "bg-gradient-end group-hover:bg-gradient-start"} group-hover:-skew-x-12`}></span>
